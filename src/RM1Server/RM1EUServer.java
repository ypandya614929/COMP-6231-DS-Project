...
public class RM1EUServer {
	...
	public RM1EUServer() {
		...
		Runnable eu = () -> {
			serverConnection(Constants.RM1_EU_SERVER_PORT);
		};
		Thread t = new Thread(eu);
		t.start();
		Runnable usd = () -> {
			sendserverData(Constants.RM1_EU_SEND_DATA_PORT);
		};
	...
	synchronized public String getPlayerStatus(String username, String password, String ip) {
		...
		if (adminObj.userName.equals("Admin") && adminObj.password.equals("Admin")) {
			...
			output += " ";
			output += DatafromOtherIP(username, password, ip, Constants.RM1_NA_SERVER_PORT, "getUDPPlayerStatus");
			return output;
		}
	...
	public void sendserverData(int port){
		...
		while (true) {
			try {
				...
				logger.info("== Sending Data of Europe from RM1 to RM3 ==");
				ds.send(dp1);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
