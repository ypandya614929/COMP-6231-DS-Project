...
public class RM1NAServer {
	 ...
	 * @param naIp ip of the RM1Server
	 */
	 ...
	 */
	public RM1NAServer() {
		addLog("logs/RM1_NA.txt", "RM1_NA");
		this.adminserverData = new ConcurrentHashMap<>();
		this.playerserverData = new ConcurrentHashMap<>();
		Runnable na = () -> {
			serverConnection(Constants.RM1_NA_SERVER_PORT);
		};
		Thread t = new Thread(na);
		t.start();
		Runnable usd = () -> {
			sendserverData(Constants.RM1_NA_SEND_DATA_PORT);
		};
	...
	synchronized public String getPlayerStatus(String username, String password, String ip) {
		...
		if (adminObj.userName.equals("Admin") && adminObj.password.equals("Admin")) {
			...
			output += DatafromOtherIP(username, password, ip, Constants.RM1_EU_SERVER_PORT, "getUDPPlayerStatus");
			output += " ";
			output += DatafromOtherIP(username, password, ip, Constants.RM1_AS_SERVER_PORT, "getUDPPlayerStatus");
			return output;
		}
	...
	public void sendserverData(int port){
		...
		while (true) {
			try {
				...
				logger.info("== Sending Data of North America from RM1 to RM3 ==");
				ds.send(dp1);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
	...
	public void serverConnection(int port) {
		...
		while (true) {
			try {
				...
				int udp_port = Constants.RM1_PORT;
