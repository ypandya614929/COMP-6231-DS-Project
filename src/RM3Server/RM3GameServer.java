...
import RM3Server.GenerateRequest;
...
public class RM3GameServer {
	...
	public void updateData() throws SocketException {
		DatagramSocket ds = null;
		try {
			
			ds = new DatagramSocket(Constants.RM3_FAULT_SERVER_PORT);
			...
			while (true) {
				...
				if (!isNullOrEmpty(temp)) {
					String server = temp.split("#")[0];
					String data = temp.split("#")[1];
					if (server.trim().equals("EU")) {
						port = Constants.RM3_EU_FAULT_SERVER_PORT;
					}
					if (server.trim().equals("AS")) {
						port = Constants.RM3_AS_FAULT_SERVER_PORT;
					}
					if (server.trim().equals("NA")) {
						port = Constants.RM3_NA_FAULT_SERVER_PORT;
					}
					data = data.trim();
					InetAddress ia = InetAddress.getByName(Constants.LOCALHOST);
					new DatagramSocket().send(new DatagramPacket(data.getBytes(), data.length(), ia, port));
				}
