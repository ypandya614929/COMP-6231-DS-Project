...
public class RM3NAServer {
	...
	public RM3NAServer() {
		...
		Runnable na = () -> {
			serverConnection(Constants.RM3_NA_SERVER_PORT);
		};
		Thread t = new Thread(na);
		t.start();
		Runnable usd = () -> {
			updateserverData(Constants.RM3_NA_FAULT_SERVER_PORT);
		};
	...
	public void updateserverData(int port) {
		...
		while (true) {
			try {
				...
				data = data.trim();
				if (data!="" || data!=null) {
					logger.info("== Started North America Server Restoring. ==");
					...
					logger.info("== North America Server Restored. ==");
				}
