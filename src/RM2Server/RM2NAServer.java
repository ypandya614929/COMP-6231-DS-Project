...
public class RM2NAServer {
	...
	public RM2NAServer() {
		...
		t.start();
		Runnable usd = () -> {
			sendserverData(Constants.RM2_NA_SEND_DATA_PORT);
		};
	...
	synchronized public String getPlayerStatus(String username, String password, String ip) {
		...
		if (adminObj.userName.equals("Admin") && adminObj.password.equals("Admin")) {
			...
			output += DatafromOtherIP(username, password, ip, Constants.RM2_EU_SERVER_PORT, "getUDPPlayerStatus");
			output += " ";
			output += DatafromOtherIP(username, password, ip, Constants.RM2_AS_SERVER_PORT, "getUDPPlayerStatus");
			return output;
		}
	...
	public void sendserverData(int port){
		...
		while (true) {
			try {
				...
				logger.info("== Sending Data of North America from RM2 to RM3 ==");
				ds.send(dp1);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
	...
	public void serverConnection(int port) {
		...
		while (true) {
			try {
				...
				if (udp_port==Constants.RM2_PORT) {
