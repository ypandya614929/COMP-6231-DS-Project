...
public class RM2EUServer {
	...
	public RM2EUServer() {
		...
		t.start();
		Runnable usd = () -> {
			sendserverData(Constants.RM2_EU_SEND_DATA_PORT);
		};
	...
	synchronized public String getPlayerStatus(String username, String password, String ip) {
		...
		if (adminObj.userName.equals("Admin") && adminObj.password.equals("Admin")) {
			...
			output += " ";
			output += DatafromOtherIP(username, password, ip, Constants.RM2_NA_SERVER_PORT, "getUDPPlayerStatus");
			return output;
		}
	...
	public void sendserverData(int port){
		...
		while (true) {
			try {
				...
				logger.info("== Sending Data of Europe from RM2 to RM3 ==");
				ds.send(dp1);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
